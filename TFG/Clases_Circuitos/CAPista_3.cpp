...
// FUNCIÓN: CAPista_3::CAPista_3(float longitud)
//
// PROPÓSITO: Crear la pistas curvas interior, estandar y exterior hacia la derecha
//            diferenciadas por los radios y los angulos recibido por parametro.
//
CAPista_3::CAPista_3(float r1, float r2, float ang)
{
	this->ang = ang;
	int num_p = 20;
	...
	if (ang == 22.5f) {
		if (r1 == 0.37f && r2 == 0.526f){
			a = 2.6411f; b = 1.0f - 0.28608f; c = 2.6446f;
		}
		else {
			a = 2.554f; b = 1.0f - 0.30077f; c = 2.6f;
		}
	  angulo_objetivo = 50.0f;
	}
	else {
		if (r1 == 0.058f && r2 == 0.214f) {
			a = 4.3557f; b = 1.0f - 0.41592f; c = 4.38f;
			angulo_objetivo = 60.0f;
		}
		else {
			a = 3.62f; b = 1.0f - 0.51478f; c = 3.64f;
	...
	for (int i = 0; i < num_p; i++) {
		...
		glm::vec3 p1 = { r_m - (r1 * Cos_alfa) , 0.0f, -(r1 * Sin_alfa) };
		float tx1 = a * p1.x + b;
		float ty1 = c * p1.z;
		vertices[verticesIndex] = { p1, {0.0f,1.0f,0.0f}, {tx1,ty1} };
		verticesIndex++;

		glm::vec3 p2 = { r_m - (r2 * Cos_alfa) , 0.0f, -(r2 * Sin_alfa) };
		tx1 = a * p2.x + b;
		ty1 = c * p2.z;
...
// FUNCIÓN: CAPista_3::estaDentro(int slot, float distancia)
//
// PROPÓSITO: Comprobar si el coche esta dentro de unas de las rarunas de la pista mediante la distancia.
//
bool CAPista_3::estaDentro(CAEstado_Coche &est)
{
	float f;
	if (est.slot == 0) f = dis_Slot1;
	else f = dis_Slot2;

	if (f > est.distancia) return true;
	return false ;
}

// FUNCIÓN: CAPista_3::getCarLocation(int slot, float distancia)
//
// PROPÓSITO: Devolver la localizacion del coche segun la distancia.
//
glm::mat4 CAPista_3::getCarLocation(CAEstado_Coche est)
{
	glm::mat4 loc = getLocation();
	s = 0.0f;
	float f = 0.039f;
	...
	glm::vec3 r = glm::vec3(r_m - (s * Cos_ang), 0.0f,  - (s * Sin_ang));
...
// FUNCION: CAPista_3::getLongitud(int s)
//
// PROPÓSITO: Devolver la longitud de la pista.
//
float CAPista_3::getLongitud(int s)
{
	if (s == 0)
		return dis_Slot1;
	else
